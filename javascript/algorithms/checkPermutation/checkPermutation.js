...
function checkPermutationEfficient(str1, str2) {
  ...
  for (let i = 0; i < str1.length; i++) {
    // note str1.length === str2.length at this point, so no need to recalculate str2.length
...
assert.equal(checkPermutation("god", "dog"), true);
assert.equal(checkPermutation("cat", "dog"), false);
assert.equal(checkPermutationEfficient("cat", "dog"), false);
assert.equal(checkPermutationEfficient("cat", "dog"), false);

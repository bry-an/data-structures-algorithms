function findPattern(string, pattern) {
  ...
  while (j < string.length) {
    ...
    if (i === pattern.length - 1) {
      // if we've reached the end of the pattern string, reset the iteration variable
      j += i + 1;
      i = 0;
    }
  }
  ...
  for (let i = 0; i < frequencyArr.length; i++) {
    if (frequencyArr[i] === pattern.length) {
      // for any values in the frequency array that match the length of our pattern, it was a total match, so push to results array
      results.push(i);
    }
  }
  return results;

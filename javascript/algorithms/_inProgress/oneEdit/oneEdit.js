...
function oneEdit(string1, string2) {
  ...
  if (str1Len + 1 === str2Len || str1Len - 1 === str2Len) {
    str1Len > str2Len
      ? (result = findExtraCharacter(str1Arr, str2Arr))
      : (result = findExtraCharacter(str2Arr, str1Arr));
...
assert.equal(oneEdit("dog", "dog"), "The two strings are identical");
assert.equal(oneEdit("god", "gold"), "Insert l");
assert.equal(oneEdit("help", "kelp"), "Swap h and k");
assert.equal(
  oneEdit("help", "aFriend"),
  "The two strings are greater than one edit apart"
);
assert.equal(
  oneEdit("fffoo", "foooo"),
  "The two strings are greater than one edit apart"
